...
- SDD ledger residuals to specifically probe with real multi-candidate collisions (`.superpowers/sdd/progress.md`, Task 6): "greedy-loser outflow in many-to-one collision gets neither link nor ReviewItem (slips into spending; plan-mandated)" and "originally-ambiguous outflow can auto-link as method=rule when rivals consumed (candidate count evaluated at decision time)."
