...
4. Time the sync loosely — each adapter fetches sequentially in `run_sync` (not concurrently, since each needs its own DB-derived `since` first); wall time should scale with the sum of both sources' fetch times, not just the slower one.
