...
| **Plaid adapter** | Full alternative/complement to SimpleFIN: credentials via `moneta setup plaid`, banks linked one-at-a-time through Plaid Hosted Link (`plaid-link` prints a URL, polls until the browser flow finishes), managed with `plaid-list` / `plaid-unlink` / `plaid-relink`. Replays its full `/transactions/sync` history (≤730 days) every run; dedup absorbs the overlap. Plaid's inverted sign conventions are normalized at the adapter boundary. Supplies real account types (`type_hint`) that beat keyword inference. Per-item auth failures degrade gracefully instead of failing the sync, with a CLI hint naming the exact repair command. |
...
| 2026-07-16 | **Plaid Link update mode (`plaid-relink`)** | `moneta setup plaid-relink <item-id>` repairs an `ITEM_LOGIN_REQUIRED` item in place via Plaid Hosted Link's update mode (`create_hosted_link(..., access_token=...)`) instead of unlink + re-`plaid-link`, which used to mint a duplicate item/account set (Plaid account ids are per-item). Same `item_id`/`access_token` entry, no exchange call. Unknown item id is a clean error pointing at `plaid-list`. The `ITEM_LOGIN_REQUIRED` sync warning's hint now names `plaid-relink <item-id>` directly instead of the duplicate-risking `plaid-link`. |
...
**Low**
- Transaction categorization; Plaid cursor-based incremental sync; vesting source adapter seam; test-coverage gaps.
